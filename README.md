...
This project aims to develop a concurrent key-value store system that utilizes Remote Procedure Calls (RPC) for
communication between clients and the server. The key-value store supports basic operations such as PUT, GET, and
DELETE. The system is designed to handle multiple outstanding client requests simultaneously, ensuring thread-safe
operations and data consistency. This document outlines the system architecture, project structure, design
considerations,
